...
鼠标：点窗格聚焦（**点击提升**：被点窗格占比自动抬到 62%，只增不减）；**双击标题栏改名**；拖边界调大小（只影响共享该边界的两侧）；**窗格内按下拖动 = 文本选择**——反色高亮、松开即复制到系统剪贴板（macOS 走 pbcopy；Linux 需 xclip/xsel/wl-copy），回滚状态下也能选历史输出；滚轮回滚/移动选中。
