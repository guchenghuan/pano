...
type State struct {
	...
	// ScrollHook, when non-nil, is called with copies of the lines that
	// scrolled off the top of the main screen (never the alternate screen).
	// It is called with the state lock held and must not call back into the
	// terminal. (Added by pano for scrollback support.)
	ScrollHook func(lines [][]Glyph)

	// BellHook, when non-nil, is called on every BEL (\a) received. Called
	// with the state lock held. (Added by pano for attention support.)
	BellHook func()

	// OSCHook, when non-nil, is called with the command number and payload
	// of OSC 9/99/777 notification sequences (cmux-style); other OSC
	// commands keep their existing behavior. Called with the state lock
	// held. (Added by pano for attention support.)
	OSCHook func(num int, payload string)
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// pano: capture lines scrolling off the top of the main screen before
	// they are cleared, so the host application can keep a scrollback.
	if t.ScrollHook != nil && orig == 0 && t.mode&ModeAltScreen == 0 {
		off := make([][]Glyph, n)
		for i := 0; i < n; i++ {
			cp := make([]Glyph, len(t.lines[orig+i]))
			copy(cp, t.lines[orig+i])
			off[i] = cp
		}
		t.ScrollHook(off)
	}
