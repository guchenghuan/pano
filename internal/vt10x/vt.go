...
// SetScrollHook installs a callback receiving the lines that scroll off the
// top of the main screen. (Added by pano for scrollback support.)
func (t *terminal) SetScrollHook(fn func(lines [][]Glyph)) {
	t.State.ScrollHook = fn
}

// SetBellHook installs a callback fired on BEL. (Added by pano.)
func (t *terminal) SetBellHook(fn func()) {
	t.State.BellHook = fn
}

// SetOSCHook installs a callback receiving OSC 9/99/777 notification
// payloads. (Added by pano.)
func (t *terminal) SetOSCHook(fn func(num int, payload string)) {
	t.State.OSCHook = fn
}
